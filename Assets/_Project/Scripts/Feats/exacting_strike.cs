    ...
    public class exacting_strike : BaseAction, FeatBase
    {
        ...
        public bool Perform(Unit parent, Unit target)
        {
            ...
            if (parent.GetActionPointsRemaining() <= 0 || parent.AttacksThisTurn <= 0)
            {
                ...
                Transform Log2 = GameObject.Find("LogLayoutGroup").transform;
                GameObject prefab2 = Resources.Load<GameObject>("LogChatBox");
                GameObject textIcon2 = Instantiate(prefab2, Log2);

                textIcon2.GetComponent<TextMeshProUGUI>().text
                    = "Can't do that";
                return false;
            }
